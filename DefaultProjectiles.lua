...
OverchargeProjectile = ClassSimple {
    ...
    OnImpact = function(self, targetType, targetEntity)
        ...
        local wep = launcher:GetWeaponByLabel('AutoOverCharge')
        if not wep then
             return 
            end
        ...
        --local wep = launcher:GetWeaponByLabel('OverCharge')
        --local wep = launcher:GetWeaponByLabel('AutoOverCharge')
		--wep.Blueprint.RateOfFire = 1/self:DamageAsRateOfFire(damage)
		--LOG("changerateoffire" .. damage .. " " .. self:DamageAsRateOfFire(damage) .. tostring(wep.Blueprint.RateOfFire))
		
		--wep.Blueprint.Overcharge.RateOfFire = 1/self:DamageAsRateOfFire(damage)
		--wep.Blueprint.AutoOvercharge.RateOfFire = 1/self:DamageAsRateOfFire(damage)
	...
	DamageAsRateOfFire = function(self, damage)
        return OverchargeShared.DamageAsRateOfFire(damage)
    end,			
    ...
    UnitsDetection = function(self, targetType, targetEntity)
     -- looking for units around target which are in splash range
        ...
        for _, unit in UnitsInSphere(launcher, self:GetPosition(), 2.7, categories.MOBILE -categories.COMMAND -categories.SUBCOMMANDER) or {} do
