...
ADFTractorClaw = Class(Weapon) {
    ...
    TractorThread = function(self, target, muzzle)
        ...
        if (not IsDestroyed(target)) and (not IsDestroyed(unit)) then
			...
			if (not IsDestroyed(target)) and (not IsDestroyed(unit)) then
				local wep = target.WeaponInstances[i]
		   ...
		   LOG("-- we're at the arm, do destruction effects " .. muzzle)
            if (not IsDestroyed(target)) and (not IsDestroyed(unit)) and (not IsDestroyed(self)) then				
					-- add effects emmiters to the arm
                    trash:Destroy()
                    trash:Destroy()
                    trash:Destroy()
                    self:ResetTarget()
					LOG("add effects emmiters to the arm")
                    local army = unit:GetArmy()
						--self.Trash:Add(AttachBeamEntityToEntity(unit,'Right_Arm_Muzzle01',army, 'Right_Arm_Muzzle01', -1, '/effects/emitters/light_blue_blinking_01_emit.bp')
						--unit.Trash:Add(CreateAttachedEmitter(unit,'Right_Arm_Muzzle01',army, '/effects/emitters/light_blue_blinking_01_emit.bp'):OffsetEmitter(2, 2, 2))
						--unit.Trash:Add(CreateAttachedEmitter(unit,'UAL0401',army, '/effects/emitters/light_blue_blinking_01_emit.bp'):OffsetEmitter(0, 10, 0))
						--unit.Trash:Add(CreateAttachedEmitter(unit,'UAL0401',army, '/effects/emitters/light_blue_blinking_01_emit.bp'):OffsetEmitter(-0.04, -0.5, 1.60))
					-- draining target hp	
					LOG("-- draining target hp")
					--set current health for target and unit
					local CurrentHealthTarget = target:GetHealth()
					local CurrentHealthUnit = unit:GetHealth()
					
					-- CreateRotator(unit, bone, axis, [goal], [speed], [accel], [goalspeed])
					if muzzle == 'Right_Arm_Muzzle02' then
                    self.RightArmRotator = CreateRotator(unit, 'Right_Arm_B02', 'y')
					else
                    self.LeftArmRotator = CreateRotator(unit, 'Left_Arm_B02', 'y')
					end
                    if muzzle == 'Right_Arm_Muzzle02' then
                    unit.Trash:Add(self.RightArmRotator)
					else
                    unit.Trash:Add(self.LeftArmRotator)
					end
					if muzzle == 'Right_Arm_Muzzle02' then
					self.RightArmRotator:SetGoal(-1):SetSpeed(10)
					else
					self.LeftArmRotator:SetGoal(1):SetSpeed(10)
					end
					
					local goalR = -1
					local goalL = 1
					local emitter = CreateAttachedEmitter(unit, muzzle, unit.Army, '/effects/emitters/jammer_ambient_01_emit.bp'):ScaleEmitter(0.5)
					
					
					--loop unit target is health 0 or destroyed or GC is destroyed
						while (CurrentHealthTarget > 0) and (not IsDestroyed(target)) and (not IsDestroyed(self)) do
							WaitTicks(1)
							if muzzle == 'Right_Arm_Muzzle02' and goalR >= -70 then
							self.RightArmRotator:SetGoal(goalR - 45):SetSpeed(15)
							elseif goalL <= 70 then
							self.LeftArmRotator:SetGoal(goalL + 45):SetSpeed(15)
							end
							--do damage to target once based on mass cost to hp ratio
							target:AdjustHealth(unit, -(target.Blueprint.Defense.MaxHealth / (target.Blueprint.Economy.BuildCostMass / 20)))
							LOG("-HP dependant on mass value ")
							--Here we need to add a line to make claws rotate 60° outwards to drop the wrecks out of the way

							--If GC is at full health then increase max health by 5 and add 5 health
							if CurrentHealthUnit == unit:GetMaxHealth() then
								unit:SetMaxHealth(unit:GetMaxHealth() + 10)
								unit:AdjustHealth(unit, 10)
							else
							--if GC not at full health just add 5 health to its current health
								unit:AdjustHealth(unit, 10)
							end
						    --update current health for both target and GC before loop restarts
							CurrentHealthTarget = target:GetHealth()
							CurrentHealthUnit = unit:GetHealth()
						end
                ...
                if not IsDestroyed(unit) then 
					...
					if muzzle == 'Right_Arm_Muzzle02' then
					self.RightArmRotator:SetGoal(0):SetSpeed(25)
					else
					self.LeftArmRotator:SetGoal(0):SetSpeed(25)
					end
					emitter:Destroy()
            else 
                self:MakeVulnerable(target)
